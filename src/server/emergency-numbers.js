/**
 * Voice-server mirror of src/backend/lib/emergency.mo.
 * Last-line defense so Twilio is never asked to dial emergency destinations
 * even if a reservation was created before this policy shipped.
 */
...
function isListedShortCode(code) {
  switch (code.length) {
    case 2:
...
function isPaddedEmergency(digits, start = 0) {
  const rest = digits.slice(start);
  if (rest.length < 4 || rest.length > 6) return false;
  for (let prefixLen = 3; prefixLen <= 5 && prefixLen < rest.length; prefixLen += 1) {
...
function isBlockedNanpCode(code, isNpa) {
...
function isBlockedNanp(digits, npaStart) {
...
function remainderIsEmergency(digits, start) {
  const rest = digits.slice(start);
  return isListedShortCode(rest) || isPaddedEmergency(digits, start);
}

export function isEmergencyDestination(phone) {
